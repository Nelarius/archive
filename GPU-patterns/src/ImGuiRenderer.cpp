...
narwhal::Program*            gShader{ nullptr };
narwhal::BufferObject*       gVbo{ nullptr };
narwhal::VertexArrayObject*  gVao{ nullptr };
narwhal::Texture*            gFont{ nullptr };
...
std::uint32_t ImGuiRenderer::refCount_ = 0u;
...
#ifdef _WIN32
    SDL_Window* window = window_.SDLwindow();
    NARWHAL_ASSERT(window != nullptr);
...
void ImGuiRenderer::createDeviceObjects_() {
    if (!gShader) {
        ...
        shaders.emplaceBack(fileToString("data/panel.vert.glsl"), GL_VERTEX_SHADER);
        shaders.emplaceBack(fileToString("data/panel.frag.glsl"), GL_FRAGMENT_SHADER);
        gShader->link(shaders);
    }
    if (!gVbo) {
        gVbo = new BufferObject(GL_ARRAY_BUFFER);
    }
    if (!gVao) {
        auto factory = VertexArrayObjectFactory{ gVbo, gShader };
        // 2 * 4 bytes
        ...
        gVao = new VertexArrayObject(0);
        *gVao = factory.getVao();
    }
    if (!gFont) {
        gFont = new Texture(GL_TEXTURE_2D);
